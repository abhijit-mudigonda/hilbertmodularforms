...
// hack begins
import "../diamond.m" : operator, GetHeckeMatrix;
...
function basis_matrix(M)
  if not assigned M`basis_matrix then
    ...
    then
        ...
        // if M is an ambient space, then the Hecke matrices on the space
        // are exactly the Hecke matrices on the H^{1,0}, which are computed
        // by basis_of_plus_subspace.
        plus_basis, minus_basis := basis_of_plus_subspace(M);
        M`basis_matrix := plus_basis;
        // If T is a matrix on the total space, I think M`basis_matrix_inv is 
        // computed such that M`basis_matrix * T * M`basis_matrix_inv gives 
        // the restriction of T to the plus subspace.
...
function hecke_algebra(M : generator:=false, minimal:=true)
  ...
  if not use_cached_hecke_algebra then
    ...
    repeat
      ...
      // matrix algebra, i.e. F-subalgebra of M_d(F), generated by Hecke matrices
 ...
 computations with M will be done relative to this basis.}
  ...
  if not M`hecke_matrix_field_is_minimal then
    ...
    for P in Keys(M`Hecke) do
      TP, p_rep := Explode(M`Hecke[P]);
      print "----------------------------------------------------------------------------";
      print "hoiiiiii", BaseRing(TP), Discriminant(BaseRing(TP));
      print "boiiiiii", H;
      print "basefield chi", BaseField(BaseRing(TP)), Discriminant(BaseField(BaseRing(TP)));
      IsSubfield(H, BaseRing(TP));
      IsSubfield(H, BaseField(BaseRing(TP)));
      print "TP", TP;
      print "norm p_rep", Norm(p_rep);
      print TP[2][1] in H;
      print "BaseRing(TP)", BaseRing(TP);
      print "BaseField(BaseRing(TP))", BaseField(BaseRing(TP));
      print "weight base field", TopAmbient(M)`weight_base_field;
      print "H", H;
      assert IsIsomorphic(BaseField(BaseRing(TP)), TopAmbient(M)`weight_base_field);
      print [x in H : x in Eltseq(TP)];
      assert &and[x in H : x in Eltseq(TP)];
...
{Given an eigenform f in a new space of Hilbert modular forms, this
 returns the eigenvalue for the Hecke operator at the prime P}
  ...
  // TODO abhijitm for debug
  // This should occur only when GaloisDescent is off in ModFrmHilD, which 
  // is never in anything I'm doing. 
...
{Returns the newforms in M that have rational Fourier coefficients}
  ...
  print "asdfasdfasdfasdfasdfasdfasdfasdf NewformsOfDegree1";
