        ...
        for psi in Elements(D) do
          ...
          if flag then
            ...
            for chi in Elements(H) do
              if IsCompatibleWeight(chi, [1,2]) then
                print "hoiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii", IdealOneLine(N), chi, HeckeCharLabel(chi);
              else
                print "Found a Grossencharacter but there are no HMFs here!";
              end if;
            end for;
          end if;
        end for;
