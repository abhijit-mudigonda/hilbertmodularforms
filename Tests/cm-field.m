import "ModFrmHilD/HMFGrossenchar.m" : eval_nonalg_inf_type_at_unit;
  ...
  if b then
    if (not IsIsomorphic(FixedField(K, [tau]), F)) then
      ...
      for k_hmf in k_hmfs do
        ...
        assert &and[eval_nonalg_inf_type_at_unit(X, eps) eq EvaluateNoncompactInfinityType(X, eps) :
          eps in UnitsGenerators(K : exclude_torsion:=false)];
      end for;
    end if;
  end if;
end procedure;
